...
- シェーダーバイナリ・コマンドバッファ・デスクリプタ
  ...
  - 今回調査したものではないライブラリでこれらのメモリ領域をバッファで扱うタイプのライブラリを確認している。
  - 汎用性を考えるとどう抽象化していいか悩ましい。
  - コマンドバッファとデスクリプタはランタイムで生成するものだから特別扱いしなくても良いとして問題はシェーダーバイナリ。
  - シェーダーバイナリは頂点やインデックスと同様にファイルから読み込んできてそのままバッファ内に配置して使うケースもある。
  - バッファの特性にシェーダーバイナリを指定できればいい？
  - あとはシェーダーオブジェクト生成でバッファも指定できるようにしておけばいける？？
- リソースオブジェクト
...
- キューについて
  - 特性をした生成は DX12 形式で統一するとシンプルでよさそう。

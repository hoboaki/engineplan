...
- ロムをビルドする
    ...
    - ビルド時に渡される情報
        ...
        - システムインクルードパスのリスト
        - 作業に使うフォルダのパス
            - $(ProjectRoot)/.Build/$(Platform)_$(BuildTarget)_$(BuildVersion)/Edt/ (エディットモード)
            - $(ProjectRoot)/.Build/$(Platform)_$(BuildTarget)_$(BuildVersion)/App/ (アプリケーションモード)
            - DevkitWinだとこういうフォルダを下に作る
                - Bin => バイナリフォルダ（成果物置き場・起動時にCodeとData以下からコピーされる）
                - Code => コードビルド領域
                - Data => データコピー領域
            - もしかしたらこういうフォルダの必要性もでてくるかも（プラットフォームやビルドターゲットに依存しないキャッシュ置き場）
                - $(ProjectRoot)/.Cache/Builder/$(Builder)/
- ビルドしたロムを起動する
- 実行中のロムを停止する
- ビルドしたロムを発行（Publish）する
    - 指定ディレクトリにアプリケーションモードのロムパッケージを配置する

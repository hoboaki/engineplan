...
struct Demo {
    ...
    vk::Bool32 check_layers(uint32_t,
                            const char* const*,
                            uint32_t,
                            vk::LayerProperties*);
    void cleanup();
    ...
    bool loadTexture(const char*,
                     ...
                     int32_t*,
                     int32_t*);
    bool memory_type_from_properties(uint32_t,
                                     vk::MemoryPropertyFlags,
                                     uint32_t*);
...
static void registry_handle_global(void* data,
    ...
    if (strcmp(interface, "wl_compositor") == 0) {
        demo->compositor = (wl_compositor*)wl_registry_bind(
...
Demo::Demo()
    :
      ...
      window{nullptr},
      minsize(
          POINT{0, 0}),  // Use explicit construction to avoid MSVC error C2797.
#endif
...
void Demo::build_image_ownership_cmd(uint32_t const& i) {
    ...
    auto result = swapchain_image_resources[i].graphics_to_present_cmd.begin(
        &cmd_buf_info);
...
vk::Bool32 Demo::check_layers(uint32_t check_count,
...
void Demo::draw_build_cmd(vk::CommandBuffer commandBuffer) {
    ...
    if (separate_present_queue) {
        ...
        commandBuffer.pipelineBarrier(vk::PipelineStageFlagBits::eBottomOfPipe,
                                      vk::PipelineStageFlagBits::eBottomOfPipe,
                                      vk::DependencyFlagBits(), 0, nullptr, 0,
...
void Demo::init(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (strcmp(argv[i], "--c") == 0 && frameCount == UINT32_MAX &&
            i < argc - 1 && sscanf(argv[i + 1], "%" SCNu32, &frameCount) == 1) {
            i++;
            continue;
        }
    ...
    mat4x4_perspective(projection_matrix, (float)degreesToRadians(45.0f), 1.0f,
                       0.1f, 100.0f);
...
void Demo::init_vk() {
    ...
    if (validate) {
        ...
        if (instance_layer_count > 0) {
            ...
            validation_found =
                check_layers(ARRAY_SIZE(instance_validation_layers),
    ...
    uint32_t gpu_count;
    result = inst.enumeratePhysicalDevices(
        &gpu_count, static_cast<vk::PhysicalDevice*>(nullptr));
    VERIFY(result == vk::Result::eSuccess);
    ...
    if (gpu_count > 0) {
        ...
        result =
            inst.enumeratePhysicalDevices(&gpu_count, physical_devices.get());
        VERIFY(result == vk::Result::eSuccess);
    ...
    gpu.getQueueFamilyProperties(
        &queue_family_count, static_cast<vk::QueueFamilyProperties*>(nullptr));
...
void Demo::init_vk_swapchain() {
    ...
    auto result = gpu.getSurfaceFormatsKHR(
        surface, &formatCount, static_cast<vk::SurfaceFormatKHR*>(nullptr));
    VERIFY(result == vk::Result::eSuccess);
...
void Demo::prepare() {
    ...
    for (uint32_t i = 0; i < swapchainImageCount; ++i) {
        result = device.allocateCommandBuffers(
            &cmd, &swapchain_image_resources[i].cmd);
        VERIFY(result == vk::Result::eSuccess);
    }
    ...
    if (separate_present_queue) {
        ...
        result = device.createCommandPool(&present_cmd_pool_info, nullptr,
                                          &present_cmd_pool);
        VERIFY(result == vk::Result::eSuccess);
...
void Demo::prepare_buffers() {
    ...
    result = gpu.getSurfacePresentModesKHR(
        surface, &presentModeCount, static_cast<vk::PresentModeKHR*>(nullptr));
    VERIFY(result == vk::Result::eSuccess);
    ...
    swapchain_image_resources.reset(
        new SwapchainImageResources[swapchainImageCount]);
...
void Demo::prepare_cube_data_buffers() {
    ...
    for (unsigned int i = 0; i < swapchainImageCount; i++) {
        auto result = device.createBuffer(
            &buf_info, nullptr, &swapchain_image_resources[i].uniform_buffer);
        VERIFY(result == vk::Result::eSuccess);

        vk::MemoryRequirements mem_reqs;
        device.getBufferMemoryRequirements(
            swapchain_image_resources[i].uniform_buffer, &mem_reqs);
        ...
        result = device.allocateMemory(
            &mem_alloc, nullptr, &swapchain_image_resources[i].uniform_memory);
        VERIFY(result == vk::Result::eSuccess);
        ...
        memcpy(swapchain_image_resources[i].uniform_memory_ptr, &data,
               sizeof data);
...
void Demo::prepare_descriptor_layout() {
    ...
    result = device.createPipelineLayout(&pPipelineLayoutCreateInfo, nullptr,
                                         &pipeline_layout);
    VERIFY(result == vk::Result::eSuccess);
}
...
void Demo::prepare_descriptor_set() {
    ...
    for (unsigned int i = 0; i < swapchainImageCount; i++) {
        auto result = device.allocateDescriptorSets(
            &alloc_info, &swapchain_image_resources[i].descriptor_set);
        VERIFY(result == vk::Result::eSuccess);
...
void Demo::prepare_framebuffers() {
    ...
    for (uint32_t i = 0; i < swapchainImageCount; i++) {
        ...
        auto const result = device.createFramebuffer(
            &fb_info, nullptr, &swapchain_image_resources[i].framebuffer);
        VERIFY(result == vk::Result::eSuccess);
    }
}
...
#include "cube.frag.inc"
    ...
    frag_shader_module =
        prepare_shader_module(fragShaderCode, sizeof(fragShaderCode));
...
void Demo::prepare_pipeline() {
    ...
    auto result =
        device.createPipelineCache(&pipelineCacheInfo, nullptr, &pipelineCache);
    VERIFY(result == vk::Result::eSuccess);
...
void Demo::prepare_render_pass() {
    ...
    vk::SubpassDependency const dependencies[2] = {
        vk::SubpassDependency()  // Depth buffer is shared between swapchain
                                 // images
...
vk::ShaderModule Demo::prepare_shader_module(const uint32_t* code,
                                             size_t size) {
...
void Demo::prepare_texture_buffer(const char* filename,
    ...
    auto result =
        device.createBuffer(&buffer_create_info, nullptr, &tex_obj->buffer);
    VERIFY(result == vk::Result::eSuccess);
    ...
    vk::MemoryPropertyFlags requirements =
        ...
        vk::MemoryPropertyFlagBits::eHostCoherent;
    auto pass =
        memory_type_from_properties(mem_reqs.memoryTypeBits, requirements,
                                    &tex_obj->mem_alloc.memoryTypeIndex);
    VERIFY(pass == true);

    result =
        device.allocateMemory(&tex_obj->mem_alloc, nullptr, &(tex_obj->mem));
    VERIFY(result == vk::Result::eSuccess);
    ...
    if (!loadTexture(filename, (uint8_t*)data.value, &layout, &tex_width,
                     &tex_height)) {
    ...
    for (uint32_t i = 0; i < texture_count; i++) {
        if ((props.linearTilingFeatures &
            ...
            set_image_layout(textures[i].image, vk::ImageAspectFlagBits::eColor,
                             ...
                             textures[i].imageLayout, vk::AccessFlagBits(),
            ...
            cmd.copyBufferToImage(staging_texture.buffer, textures[i].image,
        ...
        auto result =
            device.createSampler(&samplerInfo, nullptr, &textures[i].sampler);
        VERIFY(result == vk::Result::eSuccess);
...
#include "cube.vert.inc"
    ...
    vert_shader_module =
        prepare_shader_module(vertShaderCode, sizeof(vertShaderCode));
...
void Demo::set_image_layout(vk::Image image,
...
void Demo::update_data_buffer() {
    ...
    memcpy(swapchain_image_resources[current_buffer].uniform_memory_ptr,
           (const void*)&MVP[0][0], sizeof(MVP));
}

/* Convert ppm image data from header file into RGBA texture image */
#include "lunarg.ppm.h"
bool Demo::loadTexture(const char* filename,
                       uint8_t* rgba_data,
                       vk::SubresourceLayout* layout,
                       int32_t* width,
                       int32_t* height) {
...
bool Demo::memory_type_from_properties(
    uint32_t typeBits,
...
void Demo::create_xlib_window() {
    ...
    xlib_window = XCreateWindow(
        display, RootWindow(display, vInfoTemplate.screen), 0, 0, width, height,
...
vk::Result Demo::create_display_surface() {
    ...
    plane_props = (vk::DisplayPlanePropertiesKHR*)malloc(
        sizeof(vk::DisplayPlanePropertiesKHR) * plane_count);
    ...
    for (plane_index = 0; plane_index < plane_count; plane_index++) {
        ...
        result = gpu.getDisplayPlaneSupportedDisplaysKHR(
            plane_index, &supported_count, nullptr);
        VERIFY(result == vk::Result::eSuccess);
        ...
        supported_displays =
            (vk::DisplayKHR*)malloc(sizeof(vk::DisplayKHR) * supported_count);
    ...
    gpu.getDisplayPlaneCapabilitiesKHR(mode_props.displayMode, plane_index,
                                       &planeCaps);
    ...
    while (!done) {
        if (demo.pause) {
            ...
            if (!succ) {
                ...
                ERR_EXIT("WaitMessage() failed on paused demo",
                         "event loop error");
            }
        }
...
static void demo_main(struct Demo& demo,
                      void* caMetalLayer,
                      int argc,
                      const char* argv[]) {

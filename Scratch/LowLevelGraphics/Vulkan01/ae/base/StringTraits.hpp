...
struct StringTraits
{
    ...
    static const WriteResult NCopyStrict(CharType* buffer, pword_t bufferLength, const CharType* str);

    /// @brief NCopyと同様の処理を行い追加でエラーチェックをする。
    /// @param buffer       格納先バッファ。
    /// @param bufferLength 格納先バッファの配列長。バイト長ではない。1 <= bufferLength。
    /// @param str          コピーする文字列。
    /// @see NCopy
    /// @details 
    /// バッファ長が不足した場合はAssertで落とします。@n
    /// Assertが無効な場合は終端文字を0バイト目に設定します。
    static const WriteResult NCopy(CharType* buffer, pword_t bufferLength, const CharType* str);
    ...
    static const WriteResult SNPrintfStrict(CharType* buffer, pword_t bufferLength, const CharType* format, ...);
    ...
    /// バッファ長が不足した場合はAssertで落とします。@n
    /// Assertが無効な場合は終端文字を0バイト目に設定します。
    static const WriteResult SNPrintf(CharType* buffer, pword_t bufferLength, const CharType* format, ...);
    ...
    static const WriteResult VSNPrintfStrict(CharType* buffer, pword_t bufferLength, const CharType* format, va_list arg);
    ...
    /// バッファ長が不足した場合はAssertで落とします。@n
    /// Assertが無効な場合は終端文字を0バイト目に設定します。
    static const WriteResult VSNPrintf(CharType* buffer, pword_t bufferLength, const CharType* format, va_list arg);
};
//@}

...
/// @details
/// 継承して使用します。@n
/// 継承先のコンストラクタで SetInstance() を呼びデストラクタで UnsetInstance()
/// を呼ぶようにしてください。 @n
...
public:
    /// @name コンストラクタとデストラクタ
    //@{
    ExplicitSingleton() { AE_BASE_ASSERT(!IsCreated()); }
    ~ExplicitSingleton() { AE_BASE_ASSERT(!IsCreated()); }
    //@}
    ...
    //@{
    static inline T&
    Instance(); ///< 作成済みのインスタンスにアクセス。 @details
                ///< もし作成していない状態でこの関数を呼ぶとエラーになります。

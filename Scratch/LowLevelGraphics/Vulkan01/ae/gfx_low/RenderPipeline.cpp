    ...
    // Pipeline
    {
        ...
        const auto rasterizationInfo =
            ::vk::PipelineRasterizationStateCreateInfo()
                ...
                .setPolygonMode(InternalEnumUtil::ToPolygonMode(createInfo.RasterizerInfo().FillMode()))
                .setCullMode(InternalEnumUtil::ToCullModeFlags(createInfo.RasterizerInfo().CullMode()))
                .setFrontFace(InternalEnumUtil::ToFrontFace(createInfo.RasterizerInfo().FrontFace()))
                .setDepthBiasEnable(VK_FALSE)
                .setLineWidth(1.0f);

        const auto multisampleInfo = ::vk::PipelineMultisampleStateCreateInfo();

        auto toStencilOpState = [](const PipelineDepthStencilInfo& depthStencilInfo, const StencilOpInfo& stencilOpInfo) {
            return ::vk::StencilOpState()
                .setFailOp(
                    InternalEnumUtil::ToStencilOp(stencilOpInfo.FailOp()))
                .setDepthFailOp(
                    InternalEnumUtil::ToStencilOp(stencilOpInfo.DepthFailOp()))
                .setPassOp(
                    InternalEnumUtil::ToStencilOp(stencilOpInfo.PassOp()))
                .setCompareOp(
                    InternalEnumUtil::ToCompareOp(stencilOpInfo.CompareOp()))
        ...
        const auto depthStencilInfo =
            ::vk::PipelineDepthStencilStateCreateInfo()
                .setDepthTestEnable(createInfo.DepthStencilInfo().DepthTestEnable() ? VK_TRUE : VK_FALSE)
                .setDepthWriteEnable(createInfo.DepthStencilInfo().DepthWriteEnable() ? VK_TRUE : VK_FALSE)
                .setDepthCompareOp(InternalEnumUtil::ToCompareOp(createInfo.DepthStencilInfo().DepthCompareOp()))
                .setDepthBoundsTestEnable(VK_FALSE)
                .setStencilTestEnable(createInfo.DepthStencilInfo().StencilTestEnable() ? VK_TRUE : VK_FALSE)
                .setFront(toStencilOpState(createInfo.DepthStencilInfo(), createInfo.DepthStencilInfo().FrontFaceStencilOpInfo()))
                .setBack((toStencilOpState(createInfo.DepthStencilInfo(), createInfo.DepthStencilInfo().BackFaceStencilOpInfo())));

        std::array<::vk::PipelineColorBlendAttachmentState, Device::SupportedAttachmentCountMax_> colorBlendAttachments;
        for (int i = 0; i < createInfo.RenderPassSpecInfo().RenderTargetCount(); ++i) {
        ...
        const auto colorBlendInfo = ::vk::PipelineColorBlendStateCreateInfo()
                                        .setAttachmentCount(createInfo.RenderPassSpecInfo().RenderTargetCount())

...
{
    static typename PodStruct<T>::ValueType MaxValue()
    {
        return std::numeric_limits<typename PodStruct<T>::ValueType>::max();
    } ///< 最大値を取得する。 @return 最大値。
    static typename PodStruct<T>::ValueType MinValue()
    {
        return std::numeric_limits<typename PodStruct<T>::ValueType>::min();
    } ///< 最小値を取得する。 @return 最小値。

    /// ShortStringに変換する。@return 変換された文字列。
    const ShortString ToShortString() const
    {
        return ::ae::base::ShortStringFactory::Create(PodStruct<T>::ReadRef());
    }
